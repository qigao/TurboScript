...
spec("coro_module") {
    ...
    describe("integration") {
        ...
        it("should call TurboScript function returning single value") {
            exprtk_env_t env;
            exprtk_env_init(&env);
            
            /* Define a simple function that returns a value */
            const char *script = "func test_func() { return 999; }";
            exprtk_node_t *ast = exprtk_parse(script, strlen(script));
            check_not_null(ast);
            
            /* Execute to register the function */
            exprtk_eval(ast, &env);
            
            /* Create coroutine that will call test_func */
            coro_ctx_t *ctx = coro_ctx_create(&env, "test_func", 0);
            check_not_null(ctx);
            
            /* Resume - should complete immediately and return 999 */
            mco_result res = mco_resume(ctx->coro);
            check_int_eq(res, MCO_SUCCESS);
            
            /* Should be dead (completed) */
            mco_state state = mco_status(ctx->coro);
            check_int_eq(state, MCO_DEAD);
            check_int_eq(ctx->status, 2); /* dead */
            
            /* Check return value - could be NUMBER or INTEGER */
            int is_numeric = (ctx->return_value.type == EXPRTK_VAL_NUMBER || 
                             ctx->return_value.type == EXPRTK_VAL_INTEGER);
            check(is_numeric);
            
            if (ctx->return_value.type == EXPRTK_VAL_NUMBER) {
                check(ctx->return_value.data.number == 999.0);
            } else if (ctx->return_value.type == EXPRTK_VAL_INTEGER) {
                check(ctx->return_value.data.integer == 999);
            }
        ...
        it("should call TurboScript function with yield") {
            exprtk_env_t env;
            exprtk_env_init(&env);
            
            /* Register coro.yield function so it can be called from TurboScript
             * Note: fn_coro_yield doesn't use user_data, so we can pass NULL */
            exprtk_env_register_func(&env, "coro.yield", fn_coro_yield, NULL);
            
            /* Define a function that yields multiple times */
            const char *script = 
                "func yielder() {"
                "  coro.yield(10);"
                "  coro.yield(20);"
            ...
            /* Create coroutine that will call yielder */
            coro_ctx_t *ctx = coro_ctx_create(&env, "yielder", 0);
            check_not_null(ctx);
            
            /* First resume - should yield 10 */
            mco_result res = mco_resume(ctx->coro);
            check_int_eq(res, MCO_SUCCESS);
            ...
            check_int_eq(state, MCO_SUSPENDED);
            check_int_eq(ctx->yield_value.type, EXPRTK_VAL_NUMBER);
            check(ctx->yield_value.data.number == 10.0);
            
            /* Second resume - should yield 20 */
            res = mco_resume(ctx->coro);
            check_int_eq(res, MCO_SUCCESS);
            ...
            check_int_eq(state, MCO_SUSPENDED);
            check_int_eq(ctx->yield_value.type, EXPRTK_VAL_NUMBER);
            check(ctx->yield_value.data.number == 20.0);
            
            /* Third resume - should return 30 */
            res = mco_resume(ctx->coro);
            check_int_eq(res, MCO_SUCCESS);
            ...
            if (ctx->return_value.type == EXPRTK_VAL_NUMBER) {
                check(ctx->return_value.data.number == 30.0);
            } else if (ctx->return_value.type == EXPRTK_VAL_INTEGER) {
                check(ctx->return_value.data.integer == 30);
            }
