...
- **`turboutils`** - Salts 完整 API 参考（内存管理、字符串、文件、日志、并发、无锁数据结构）
...
- ❌ **禁止手写**：动态数组 → 用 Salts::STL 的 `vec_t` / `TURBO_VEC_DEFINE`，临时数组可用 `mem_pool_t`
- ❌ **禁止手写**：字符串拼接 → 用 `tstr`（Salts）或 `sds`（vendor）
- ❌ **禁止手写**：哈希表/集合 → 用 Salts::STL 的 `hash_map_t` / `TURBO_HASH_MAP_DEFINE` 或 `set_t` / `TURBO_SET_DEFINE`
- ❌ **禁止手写**：双端队列 → 用 Salts::STL 的 `deque_t` / `TURBO_DEQUE_DEFINE`
- ❌ **禁止手写**：文件读写 → 用 `turbo_fs`（Salts）
