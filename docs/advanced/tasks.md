...
var first = task.spawn(() => {
    return http.get("https://example.com/", map { timeout: 5000 });
});

var second = task.spawn(() => {
    return http.get("https://www.iana.org/", map { timeout: 5000 });
});
...
每个 task 拥有一个 CoroNet cancellation source。`task.cancel()` 会唤醒 `task.sleep()`、`task.join()` 以及通过 `http_request_ex()` 执行的 HTTP 等待；被取消的 HTTP transport 会从连接池丢弃且不进入 retry。取消是协作式的：纯 CPU callback 只能在下一次 `task.yield()`、task API、可取消 I/O 或 callback 返回时观察请求，runtime 不会强制销毁正在运行的栈。
